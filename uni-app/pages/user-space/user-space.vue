<template>
	<view style="height: 100%;">
		...
		<view style="height: 100%;">
			...
			<!-- ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ -->
			<template v-else-if="tabIndex === 1"  style="height: 100%;">
				<view class="a">
					
					<view v-for="(item,index) in petList" :key="index" 
					class="flex align-center p-2 mb-2 b" hover-class="bg-light" @click="petxiangqing(index)">
						...
						<view class="flex flex-column flex-1 px-2">
							...
							<text class="iconfont icon-nv container"></text>
							</text>
...
<script>
	...
	export default {
		...
		onLoad(e) {
			console.log(e);
			this.theid = e.user_id;
			// 将访问个人空间获取到的id提交到vuex
			this.$store.commit('editTheId',{
				value:this.theid
			})
		...
		methods: {
			...
			getUserInfo(){
				this.$H.post('/getuserinfo',{
					...
					// 	birthday:res.userinfo.birthday ? $T.getHoroscope(res.userinfo.birthday) : '无',
					// 	job:res.userinfo.job ? res.userinfo.job : '无',
					// 	path:res.userinfo.path ? res.userinfo.path : '无',
					// 	qg:emotionArray[res.userinfo.qg],
					// };
					this.userinfo.userpic = res.userpic;
					this.userinfo.username = res.username;
					this.userinfo.sex = res.userinfo.sex;
					this.userinfo.age = res.userinfo.age;
					this.userinfo.isFollow = res.fens.length;
					this.userinfo.isblack = res.blacklist.length;
					this.userinfo.regtime = $T.dateFormat(new Date(res.create_time * 1000), '{Y}-{MM}-{DD}');
					this.userinfo.birthday = res.userinfo.birthday ? $T.getHoroscope(res.userinfo.birthday) : '无';
					this.userinfo.job = res.userinfo.job ? res.userinfo.job : '无';
					this.userinfo.path = res.userinfo.path ? res.userinfo.path : '无';
					this.userinfo.qg = emotionArray[res.userinfo.qg];
					console.log(res.userinfo.qg);
			...
			getPetInfo(){
				...
				uni.request({
							url: 'https://ceshi.aichong.online/api/v1/user/'+this.user_id+'/pet', 
							method:'GET',
							data:{},
							success: (res) => {
								console.log(res);
								...
								// 将通过个人空间访问到的宠物信息（也有可能是自己的）提交到vuex
								this.$store.commit('editThePet',{
									value:res.data.data.list
								})
								console.log($store.state.ThePet);
							},
							fail:(res) =>{
			...
			petxiangqing(id){
				console.log('宠物详情');
				uni.navigateTo({
					url: '../petxiangqing/petxiangqing?id='+id
				});
			}
			/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
