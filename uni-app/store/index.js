...
export default new Vuex.Store({
	state:{
		...
		// 聊天会话列表
		chatList:[],
		// 要修改的宠物保存的旧的值，在chongwuxiangqing界面添加的
		pet:{},
		// 在my界面中保存的部分宠物信息
		petList:[]
	},
	...
	mutations:{
		...
		//修改通过个人空间访问到的宠物信息（也有可能是自己的）
		editThePet(state,{value }){
			state.ThePet = value
		},
		// 修改用户信息(手机号,邮箱,密码)
		...
		//保存宠物信息，在my界面全体宠物列表，并且不全
		editPetInfo(state,{value }){
			state.petList= value;
			uni.setStorageSync('petList', JSON.stringify(state.petList));
		},
		editTheId(state,{value }){
			state.theid = value
		},
		
		// 在chongwuxiangqing页的时候进行保存某一个宠物具体的信息
		editPetXiugai(state,{value }){
			state.pet = value;
		},
		// 修改资料
